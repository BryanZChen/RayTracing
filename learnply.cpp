...
Polyhedron::Polyhedron() : orientation(0), seed(-1), center(0.0, 0.0, 0.0), radius(0.0), area(0.0), in_ply(NULL)
{

}

Polyhedron::Polyhedron(std::vector<Vertex*>& verts, std::vector<Triangle*>& tris, bool re_index) : orientation(0), seed(-1), center(0.0, 0.0, 0.0), radius(0.0), area(0.0), in_ply(NULL)
{
    vlist = verts;
    tlist = tris;
    if (re_index)
...
******************************************************************************/
Polyhedron::Polyhedron(FILE *file) : orientation(0), seed(-1), center(0.0, 0.0, 0.0), radius(0.0), area(0.0), in_ply(NULL)
{
  int i,j;
  int elem_count;
  char *elem_name;
...
void Polyhedron::calc_bounding_sphere()
{
  ...
  for (int i=0; i<nverts(); i++) 
  {
	...
	{
		if (vlist[i]->pos[0] < minV[0]) { minV[0] = vlist[i]->pos[0]; }
        if (vlist[i]->pos[0] > maxV[0]) { maxV[0] = vlist[i]->pos[0]; }
        if (vlist[i]->pos[1] < minV[1]) { minV[1] = vlist[i]->pos[1]; }
        if (vlist[i]->pos[1] > maxV[1]) { maxV[1] = vlist[i]->pos[1]; }
        if (vlist[i]->pos[2] < minV[2]) { minV[2] = vlist[i]->pos[2]; }
        if (vlist[i]->pos[2] > maxV[2]) { maxV[2] = vlist[i]->pos[2]; }
	}
  }
  center = (minV + maxV) * 0.5;
  //radius = (center - minV).norm();
  radius = unit_vector(center - minV).length();
}
